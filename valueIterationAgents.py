...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def __init__(self, mdp, discount = 0.9, iterations = 100):
        ...
        for i in range(iterations):
            ...
            for curState in states:
                ...
                # Compute new Q value
                newValues[curState] = self.computeQValueFromValues(curState, action)
            # Memorize new values
    ...
    def computeQValueFromValues(self, state, action):
        ...
        # Calculate new Q value by adding up all values of possible transitions
        for transState, probability in transStates:
            qVal += probability * (self.mdp.getReward(state, action, transState)
                                   + self.discount * self.values[transState])
        return qVal
    ...
    def computeActionFromValues(self, state):
        ...
        for action in posActions:
            # Get its Q value and memorize the action with maximum value
